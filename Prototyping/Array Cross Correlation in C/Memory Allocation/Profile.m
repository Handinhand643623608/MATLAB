...
function t = Profile()
	...
	init = zeros(length(ntimes), length(nsignals));
	t = struct...
	(...
		'H',                init,...
        'S',				init,...
	...
	for a = 1:length(ntimes)
		for b = 1:length(nsignals)
			
			writeline('\nProfiling Execution Times for %d Signals with %d Time Points Each:\n', nsignals(a), ntimes(b));

			s1 = randn(ntimes(a), nsignals(b));
			s2 = randn(ntimes(a), nsignals(b));

            fh = @() xcorrch(s1, s2);
			fs = @() xcorrcs(s1, s2);
            fsft = @() xcorrcsft(s1, s2);
            
            t.H(a, b) = timeit(fh, 1);
			t.S(a, b) = timeit(fs, 1);
            t.SFT(a, b) = timeit(fsft, 1);

			writeline('Heap Allocated Execution Time:		%d s', t.H(a, b));
            writeline('Stack Allocated Execution Time:      %d s', t.S(a, b));
            writeline('FT Stack Allocated Execution Time:   %d s', t.SFT(a, b));
