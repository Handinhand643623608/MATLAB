...
%LOADOBJ - The update procedure for EEG data objects loaded off of the hard drive.
%
%   SYNTAX:
%   output = eegObj.loadobj(input)
%
%   OUTPUT:
%   output:     EEGOBJ or ANYTHING
%               An object of absolutely any type that is defined in MATLAB (including user-defined types) and is stored
%               inside of a .mat file. If the inputted object is an EEG data object (of type eegObj), then this output
%               argument is also an EEG data object, but the two may not be identical to one another. For example, if an
...
%
%   INPUT:
%   input:      EEGOBJ or ANYTHING
%               An object of absolutely any type that is defined in MATLAB (including user-defined types) and is stored
%               inside of a .mat file. If this object is an EEG data object (of type eegObj), then it is run through
...
currentVersion = eegObj.LatestVersion;

% Check if the inputted object is an EEG data object
eegObjCheck = isa(input(1), 'eegObj');
if eegObjCheck
    ...
    eegObjCheck = isstruct(input(1)) && all(isfield(input(1), {'Channels', 'Fs', 'StoragePath'}));
    
    % If certain EEG-specific fields aren't present, then it can't be an EEG data object
    if ~eegObjCheck
        output = input;
        return;
    end
...
function transferProperties(eegStruct, eegData)
    propNames = fieldnames(eegStruct);
    for a = 1:length(propNames)
        eegData.(propNames{a}) = eegStruct.(propNames{a});
    end
end
