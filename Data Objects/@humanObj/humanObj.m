...
%		20141118:	Bug fix for the method IsPostprocessed where I forgot to put the stage name before the status name
%					in the postprocessing structure field access.
%		20141120:	Bug fix for missing the check in the Resampling Postprocessing data structure and the method that
%					sets it.
...
%   - Test all rewritten functionality.
    ...
    properties (AbortSet, SetAccess = public)
    ...
    methods
        ...
        %   This function is used to convert data objects into data structures. Structures are a value-type in MATLAB
        %   and as such are often easier and more intuitive to deal with in this language. Additionally, they offer a
        %   lot more flexibility in terms of dynamic field creation and removal.
        %
        %   However, there are downsides to this conversion. While structures appear not to suffer from the code bload
        %   that classes can have, most of the quality control benefits inherent to classes are lost. Examples include:
        %   type-safe methods, subclassing, resticted access to fields/methods, and validation of field modifications.
        %   Also, as a reference-type, the correct use of classes can eliminate some significant computational overhead
        %   that is incurred when using a structure (this is especially apparent for large data sets).
        %
        %   This method exists so that the user may freely work with a human data set without any restrictions or the
        %   need to work with the specialized class methods.
        %
        %   SYNTAX:
        ...
        %   OUTPUT:
        %   dataStruct:     STRUCT
        %                   A value-type human data structure in exactly the same apparent format as the inputted data
        %                   object. This structure may be editted freely but can no longer be used with any of the
        %                   methods that are associated with the original object's class.
        %
        %   INPUT:
        %   dataObject:     HUMANOBJ
    ...
    methods 
		...
		%
		%	phaseDelay:				INTEGER
		%							The time delay (in seconds) of the time series resulting from the phase delay
		%							imposed by filtration.
		%
		...
		%
		%   oldFs:          DOUBLE
		%                   The old sampling frequency (in Hertz) of the data. This is the sampling frequency that the
		%                   data had just prior to running the RESAMPLE function.
		%
		%   newFs:          DOUBLE
		%                   The new sampling frequency (in Hertz) of the data. This is the sampling frequency that was
		%                   just applied using the RESAMPLE function on the data object time series.
    ...
    methods (Static)
		...
		%   according to the fitting parameters and subtracts them from the data time series. Thus, the data that exists
		%   after calling this method are the residual time series left over from the regression.
		%
		%   Linear regression is currently a popular method of removing artifacts from data and accounting for signals
		%   that are not likely to be neuronal in origin. Partial correlation, for instance, uses this approach to
		%   control for a set of variables while estimating how two other data sets covary.
		%
		...
		%   INPUTS:
		%   dataMatrix:     2D ARRAY
		%                   A 2-dimensional array (matrix) of time series data formatted as [SIGNALS x TIME], where time
		%                   points span the columns of the matrix. While the number of signals in this array can be any
		%                   number, the number of time points (i.e. columns) must equal the number of time points in the
		%                   SIGNAL argument exactly.
		%
		%   signal:         1D ARRAY or 2D ARRAY
		%                   A vector or array of signals to be regressed from the data matrix. This argument must be
		%                   provided in the format [SIGNALS x TIME], where time points span the columns of the matrix.
		%                   Just like the DATAMATRIX argument, the number of signals here can be any number, but the
		%                   number of time points must equal the number that the data matrix contains.
		%
    ...
    methods
		...
		%ASSERTSINGLEOBJECT - Throws an error if an array of multiple data objects is detected.
			assert(numel(dataObject) == 1, 'Only one data object may be inputted at a time');
        end
    ...
    methods
        ...
        function isDetrended    = get.IsDetrended(dataObject)
            isDetrended = dataObject.IsPostprocessed('Detrending', 'IsDetrended');
        end
    ...
    methods (Access = protected)
        function isPostprocessed = IsPostprocessed(dataObject, stageName, statusName)
            ...
            if (isfield(dataObject.Postprocessing, stageName))
                if (dataObject.Postprocessing.(stageName).(statusName))
