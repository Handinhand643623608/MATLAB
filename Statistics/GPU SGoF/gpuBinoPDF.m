...
%       20140127:   Custom version of this builtin function created. Added support for GPU arrays, which required 
%                   removing most error checks on inputs. Copied over Stirling approximation error & binomial
%                   deviance term approximation functions & renamed them for use with this function.
...
% Copy over any NaNs in the inputs to the output.
...
if any(k)
    ...
    if any(~t)
        ...
        % Calculate the pdf
        lc = gpuStirlErr(n(K))-gpuStirlErr(x(K))-gpuStirlErr(n(K)-x(K)) ...
             -gpuBinoDeviance(x(K),n(K).*p(K)) ...
             -gpuBinoDeviance(n(K)-x(K),n(K).*(1.0-p(K)));
