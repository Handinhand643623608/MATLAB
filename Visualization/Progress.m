...
%   file-exchange. Unlike others, this progress bar uses multiple colors and smooth movements (with acceleration effects). It
%   is designed primarily for use in tracking progress through FOR loops.
%   
%   Multiple progress bars inside of a single window can be generated by inputting more than one bar title. This is useful
%   when tracking the progress of multiple nested loops. Each individual bar can be controlled independently of one another.
%
...
%   OUTPUT:
%   H:                  PROGRESS
%                       The handle of the progress bar object. To get the object properties, type the chosen output variable
%                       name in the console or, for more advanced properties, use the GET command on the object.
%
%   OPTIONAL INPUTS:
%   '-fast':            VERBATIM
%                       If the first input in the call to create a new progress bar is the string '-fast' (exactly as shown
%                       here), then the animation that appears to smoothly advance the progress bar will be turned off. This
%                       may be helpful in situations where you are unsure of the required time for a process or you wish to
%                       monitor the progress of a fairly rapid procedure. In such cases, the animation can introduce a
%                       considerable delay to your program and should be disabled.
%
%   '-persist':         VERBATIM
...
%                       A string (or multiple strings separated by commas) represeting the label to be applied to individual
%                       progress bars. The number of bar titles corresponds to the number of bars that are generated (i.e.
%                       three bar titles will result in three bars being generated).
...
%                   property (I don't think this is doable elegantly in MATLAB). Removed changelog entries (see SVN for the
%                   complete listing).
