function bd0 = gpuBinoDeviance(x,np)
...
%       20140127:   Copied function over to personal workspace to work with the GPU implementation of SGoF. Other than 
%                   converting inputs/outputs to GPU arrays, no changes to this code were made.


bd0=gpuArray(zeros(size(x)));
...
if any(k(:))
    ...
    v = (x(k)-np(k))./(x(k)+np(k));
    ej = 2.*x(k).*v;
    s1 = gpuArray(zeros(size(s)));
    ok = gpuArray(true(size(s)));
    j = gpuArray(0);
    while any(ok(:))
